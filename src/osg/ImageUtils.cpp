        ...
        for (int l = 0; l<depth; l+=4)
        {
            for (int r = 0; r<height; r+=4)
            {
                ...
                for (int c = 0; c<width; c+=4)
                {
                    ...
                    osg::Vec3i srcOrigin(cp.x() & 0x7, cp.y() & 0x7, cp.z() & 0x7);
                    dxtc_tool::compressedBlockOrientationConversion(srcImage->getPixelFormat(),src_block, dst_block, srcOrigin, rowDelta, columnDelta);
