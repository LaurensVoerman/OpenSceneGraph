...
int Thread::start() {
	...
	setThreadName(pd->threadName.c_str());
#endif
    if(!pd->tid) {
        return -1;
    }
...
#ifdef OPENTHREADS_THREAD_NAME
#if defined(_MSC_VER)
void OpenThreads::Thread::setThreadName(const char * threadName) {
...
#pragma pack(pop)
	Win32ThreadPrivateData *pd = static_cast<Win32ThreadPrivateData *> (_prvData);
	pd->threadName = threadName;
	DWORD dwThreadID = (DWORD)(pd->uniqueId);
	if (dwThreadID == 0) return; //thread not running yet, will set name on Thread::start()
...
#else
void OpenThreads::Thread::setName(const char * ) {}
#endif
#endif
