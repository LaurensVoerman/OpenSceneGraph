		...
		IF (${varname}_DEBUG)
			IF (NOT ${varname}_RELEASE)
				IF (${varname} STREQUAL ${varname}_DEBUG)
					SET(${varname}_DEBUG "FALSE")
					unset(${varname}_DEBUG CACHE)
					unset(${varname}_DEBUG)
				ENDIF()
			ENDIF()
		ENDIF()
        IF (NOT ${varname}_DEBUG)
            IF (${varname}_RELEASE)
			    IF (EXISTS "${${varname}_RELEASE}")
                    get_filename_component(${varname}_DEBUG_PATH ${${varname}_RELEASE} PATH)
                    get_filename_component(${varname}_DEBUG_NAME_WE ${${varname}_RELEASE} NAME_WE)
                    get_filename_component(${varname}_DEBUG_EXT ${${varname}_RELEASE} EXT)
                    find_library(${varname}_DEBUG ${${varname}_DEBUG_NAME_WE}${CMAKE_DEBUG_POSTFIX}${${varname}_DEBUG_EXT} PATHS ${${varname}_DEBUG_PATH} NO_DEFAULT_PATH)
                    IF(${varname}_DEBUG)
                        message ( "Found ${varname}_DEBUG by adding CMAKE_DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX}" )
                    ELSE(${varname}_DEBUG)
                        message ( "Could NOT find ${varname}_DEBUG by adding CMAKE_DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX} to ${${varname}}" )
                    ENDIF(${varname}_DEBUG)
				ENDIF (EXISTS "${${varname}_RELEASE}")
            ELSE (${varname}_RELEASE)
                IF (${varname} AND EXISTS "${${varname}}")
				    message ( "get_filename_component(${varname}_DEBUG_PATH ${${varname}} PATH)" )
                    get_filename_component(${varname}_DEBUG_PATH ${${varname}} PATH)
				    message ( "---" )
                    get_filename_component(${varname}_DEBUG_NAME_WE ${${varname}} NAME_WE)
                    get_filename_component(${varname}_DEBUG_EXT ${${varname}} EXT)
                    find_library(${varname}_DEBUG ${${varname}_DEBUG_NAME_WE}${CMAKE_DEBUG_POSTFIX}${${varname}_DEBUG_EXT} PATHS ${${varname}_DEBUG_PATH} NO_DEFAULT_PATH)
                    IF(${varname}_DEBUG)
                        message ( "Found ${varname}_DEBUG by adding CMAKE_DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX}" )
                    ELSE(${varname}_DEBUG)
                        message ( "Could NOT find ${varname}_DEBUG by adding CMAKE_DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX} to ${${varname}}" )
                        message ( "find_library(${varname}_DEBUG ${${varname}_DEBUG_NAME_WE}${CMAKE_DEBUG_POSTFIX}${${varname}_DEBUG_EXT} PATHS ${${varname}_DEBUG_PATH} NO_DEFAULT_PATH)" )
                    ENDIF(${varname}_DEBUG)
                ENDIF (${varname} AND EXISTS "${${varname}}")
            ENDIF (${varname}_RELEASE)
        ENDIF (NOT ${varname}_DEBUG)
...
#the installation path are differentiated for win32 that install in bib versus other architecture that install in ${OSG_INSTALL_LIBDIR}/${OSG_PLUGINS}
    IF(WIN32)
...
MACRO(SETUP_IMAGE_PLUGIN PLUGIN_NAME)
  SETUP_PLUGIN(${PLUGIN_NAME})
  SET_TARGET_PROPERTIES(${TARGET_TARGETNAME} PROPERTIES FOLDER "imagePlugin")
  #replace default symbol (osgdb_rgb_EXPORTS)
  SET_TARGET_PROPERTIES (${TARGET_TARGETNAME} PROPERTIES DEFINE_SYMBOL  "IMGPLUGIN" )
ENDMACRO(SETUP_IMAGE_PLUGIN PLUGIN_NAME)

MACRO(SETUP_PSEUDOLOADER_PLUGIN PLUGIN_NAME)
  SETUP_PLUGIN(${PLUGIN_NAME})
...
MACRO(REPLACE_IF_UPDATED OLDFILE NEWFILE)
    IF(EXISTS ${OLDFILE})
    	execute_process( COMMAND ${CMAKE_COMMAND} -E compare_files ${OLDFILE} ${NEWFILE}
...
#create and compile _concat.cpp from sources list with full path providing "#define" for "MyClass"
MACRO(OSG_COMPILE_CONCAT_GLOB_FIXMYCLASS CONCAT_SRC CONCAT_EXCLUDE)
    IF (OSG_COMPILE_CONCATENATE)
        file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "//autogenerated by cmake\n" )
        
        FOREACH(srcFile ${${CONCAT_SRC}})
    	    set_source_files_properties(${srcFile} PROPERTIES HEADER_FILE_ONLY FALSE)
			if(";${CONCAT_EXCLUDE};" MATCHES ${srcFile})
			    continue()
        	endif()
			get_filename_component(FILE_NAME_WE ${srcFile} NAME_WE)
			#define MyClass MyAlphaFuncClass
    		file(APPEND ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "#define MyClass My${FILE_NAME_WE}Class\n" )
    		file(APPEND ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "#include \"${srcFile}\"\n" )
    		file(APPEND ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "#undef MyClass\n" )
            set_source_files_properties(${srcFile} PROPERTIES HEADER_FILE_ONLY TRUE)
        ENDFOREACH(srcFile)
        REPLACE_IF_UPDATED(${CMAKE_CURRENT_BINARY_DIR}/_concat.cpp ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp)
        SET(${CONCAT_SRC}
            ${CMAKE_CURRENT_BINARY_DIR}/_concat.cpp ${${CONCAT_SRC}}
        )
    ELSE (OSG_COMPILE_CONCATENATE)
        set_source_files_properties(${CMAKE_CURRENT_BINARY_DIR}/_concat.cpp PROPERTIES HEADER_FILE_ONLY TRUE)
    ENDIF (OSG_COMPILE_CONCATENATE)
ENDMACRO()

#create and compile _concat.cpp from sources list with full path
MACRO(OSG_COMPILE_CONCAT_GLOB CONCAT_SRC CONCAT_EXCLUDE)
    IF (OSG_COMPILE_CONCATENATE)
        file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "//autogenerated by cmake\n" )
...
#create and compile _concat.cpp from sources list with relative path
MACRO(OSG_COMPILE_CONCAT_LIST CONCAT_SRC CONCAT_EXCLUDE)
    IF (OSG_COMPILE_CONCATENATE)
        file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "//autogenerated by cmake\n" )
        ...
        FOREACH(srcFile ${${CONCAT_SRC}})
    		...
    		file(APPEND ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "#include \"${CMAKE_CURRENT_SOURCE_DIR}/${srcFile}\"\n" )
            set_source_files_properties(${srcFile} PROPERTIES HEADER_FILE_ONLY TRUE)
        ENDFOREACH(srcFile)
        REPLACE_IF_UPDATED(${CMAKE_CURRENT_BINARY_DIR}/_concat.cpp ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp)
        SET(${CONCAT_SRC}
...
#create and compile _concat.cpp from sources list with relative path
MACRO(OSG_COMPILE_CONCAT_LIST_SIZEFILTER CONCAT_MAXSIZE CONCAT_SRC CONCAT_EXCLUDE)
    IF (OSG_COMPILE_CONCATENATE)
        file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "//autogenerated by cmake\n" )
        
        FOREACH(srcFile ${${CONCAT_SRC}})
		    get_filename_component(FILE_NAME_EXT ${srcFile} EXT)
			if(NOT ${FILE_NAME_EXT} STREQUAL ".cpp")
			    continue()
        	endif()
    	    set_source_files_properties(${srcFile} PROPERTIES HEADER_FILE_ONLY FALSE)
			if(";${CONCAT_EXCLUDE};" MATCHES ${srcFile})
			...
			file(SIZE "${CMAKE_CURRENT_SOURCE_DIR}/${srcFile}" SRC_FILE_SIZE)
			IF (${SRC_FILE_SIZE} GREATER ${CONCAT_MAXSIZE})
			    continue()
        	ENDIF()
    		file(APPEND ${CMAKE_CURRENT_BINARY_DIR}/_concat2.cpp "#include \"${CMAKE_CURRENT_SOURCE_DIR}/${srcFile}\"\n" )
            set_source_files_properties(${srcFile} PROPERTIES HEADER_FILE_ONLY TRUE)
